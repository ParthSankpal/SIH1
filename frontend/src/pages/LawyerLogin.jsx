...
const LawyerLogin = () => {
  ...
  e.preventDefault();
  try {
    const response = await axios.post('http://localhost:3000/api/lawyer/login-lawyer', formData);
    
    if (response.data.user) {
      // Store user-related data in session storage upon successful login
      sessionStorage.setItem('userId', response.data.user[0]._id);
      // console.log(response.data.user[0]._id, "USER_ID");
      sessionStorage.setItem('userEmail', response.data.user[0].email);
      // console.log(response.data.user[0].email, "EMAIL_ID");

      // Store other non-sensitive user data if needed
    }
    console.log(response.data);


    // Redirect to profile page upon successful login
    navigate('/lawyer/profile'); // Use history.push to navigate
  } catch (error) {
    console.log(error);
  }
};
        ...
        <form className='mt-5 pt-4' onSubmit={handleSubmit}>
          ...
          <div className='mb-3'>
            ...
            <div className='input-group'>
              <input type="email" class="form-control" name="email" onChange={handleChange} placeholder='Enter your email address' required />
          ...
          <div className='mb-1'>
            ...
            <div className='input-group'>
              <input type="password" class="form-control" name="password" onChange={handleChange} placeholder='Enter Password' required />
          ...
          <p class='d-flex justify-content-center my-2'>Don’t have an account? <a href='/lawyer/register'>Sign up</a></p>
